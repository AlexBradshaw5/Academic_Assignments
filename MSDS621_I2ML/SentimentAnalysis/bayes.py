...
class NaiveBayes621:
    ...
    def fit(self, X:np.ndarray, y:np.ndarray) -> None:
        ...
        """
        P_class1 = sum(y)/len(y)
        P_class0 = 1-P_class1
        word_class_count = [np.zeros(len(X[0])), np.zeros(len(X[0]))]
        for i in range(len(y)):
            word_class_count[int(y[i])] += X[i]   
        PW0 = (word_class_count[0]+1)/(len(y)+len([x for x in word_class_count[0] if x>0])+1)
        PW1 = (word_class_count[1]+1)/(len(y)+len([x for x in word_class_count[1] if x>0])+1)
        self.P_words = [PW0, PW1]
        self.P_classes = [P_class0, P_class1]
    ...
    def predict(self, X:np.ndarray) -> np.ndarray:
        ...
        for doc in range(len(X)): # iter through docs
            zero_sum = np.log(self.P_classes[0])
            one_sum = np.log(self.P_classes[1])       
            zero_sum += np.dot(X[doc], np.log(self.P_words[0]))
            one_sum += np.dot(X[doc], np.log(self.P_words[1]))
            predicts[doc] = int(np.round((zero_sum/one_sum)/2))
        return predicts
...
def kfold_CV(model, X:np.ndarray, y:np.ndarray, k=4) -> np.ndarray:
    ...
    for train_index, test_index in kf.split(X):
        ...
        accuracies.append(np.sum(y_test==model.predict(X_test))/len(y_test))
