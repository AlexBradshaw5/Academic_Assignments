...
def load_glove(filename):
    ...
    for line in lines:
        ...
        for i in range(1, len(line)):
            mydict[line[0]].append(float(line[i]))
        mydict[line[0]] = np.array(mydict[line[0]])
    return mydict
...
def load_articles(articles_dirname, gloves):
    ...
    for file in files:
        if(file.endswith('.txt')):
            ...
            title = txt[0]
            rest_of_text = '\n'.join(txt[1:])
            centroid = doc2vec(rest_of_text, gloves)
            lst.append((f'''{topic}/{txt_name}''', title, rest_of_text, centroid))
    return lst
...
def doc2vec(text, gloves):
    ...
    """
    ws = words(text)
    glove_words = [] #list of np.array's
    for word in ws:
        if word in gloves.keys():
            glove_words.append(gloves[word])
    return sum(glove_words)/len(glove_words)
...
def distances(article, articles):
    ...
    origin = article[3]
    for a in articles:
        dist = np.linalg.norm(origin - a[3])
        if(dist != 0):
            lst.append((dist, a))
    return lst
...
def recommended(article, articles, n):
    ...
    """
    dis = distances(article, articles) # list of tuples (distance, [filename, title, etc...])
    dis.sort() # sorts by distance
    dis = dis[:n]
    return [x[1] for x in dis]
